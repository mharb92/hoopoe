...
- Anthropic key as an env var (readable by anyone using the environment) or, preferred, batch calls routed through the existing `claude` edge function. The env var alone is not enough: `api.anthropic.com` has to be on the Custom network list as well.
