...
| R6 (C10.12) | Config is data (D118), so a mid-test change can redefine what a band, a learned word or a quiz outcome means across records already written | Frozen and hot key classes (D119), returned as two separate objects by the resolver (D234) so the class of a key is mechanical rather than a judgement at each read site; frozen keys pinned at plan start and changed only at a phase boundary; **the pinning record carries the resolved frozen document itself, not a version pointer (D238)** — a pointer has no runtime reader, so a later build would read current frozen values for a plan pinned two versions back; the version hash travels beside it for attribution; `runtime_flag` limited to an operational allowlist and never read by the client (D234) | Never fully; the pinning makes any drift attributable rather than silent |
